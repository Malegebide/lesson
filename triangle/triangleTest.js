...
QUnit.test("isosceles triangle has two equal sides", function( assert ) {
	...
	assert.equal(triangle.isIsosceles(1,1,7),false,"5,2,5 is not isosceles") ;
	assert.equal(triangle.isIsosceles(3,2,5),false,"5,2,5 is not isosceles") ;
	assert.equal(triangle.isIsosceles(0,0,1),false,"5,2,5 is not isosceles") ;
	assert.equal(triangle.isIsosceles(-1,-1,5),false,"5,2,5 is not isosceles") ;
});
QUnit.test("scalene triangle has three different sides", function( assert ) {
	...
	assert.equal(triangle.isScalene(1,2,6),false,"2,2,4 is not scalene") ;
});